...
def biLSTM(bi_lstm, inputs, batch_size=None):
    """
    Feature extraction
    ...
    """
    for cell in bi_lstm:
        outputs, states = cell.unroll(length=inputs.shape[0], inputs=inputs, layout='TNC', merge_outputs=True)
        cell.reset()
        inputs = outputs
    return inputs
